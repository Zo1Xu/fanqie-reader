...
.footer-nav{position:fixed;z-index:12;left:50%;bottom:0;display:grid;width:min(280px,100%);grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px;min-height:36px;padding:3px 8px;background:var(--vscode-terminal-background,var(--vscode-panel-background));color:var(--vscode-descriptionForeground);font-size:12px;border:0;opacity:.55;transform:translateX(-50%);transition:opacity .15s ease}.footer-nav:hover,.footer-nav:focus-within{color:var(--vscode-terminal-foreground,var(--vscode-foreground));opacity:1}.footer-nav button{min-width:0;white-space:nowrap;padding:5px 8px;border:0;border-radius:4px;background:transparent;color:inherit;cursor:pointer;font:inherit}.footer-nav button:hover:not(:disabled){background:var(--vscode-toolbar-hoverBackground)}.chapter-index{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center;color:var(--vscode-descriptionForeground);font-size:12px}
...
<main class="terminal-output" aria-label="小说阅读终端输出">
  <header class="terminal-header" aria-label="当前章节终端状态">
    ...
    <div class="terminal-line"><span class="log-info">[INFO]</span> Chapter resolved: ${escapeHtml(chapter.title)}${chapter.wordCount ? ` (${chapter.wordCount.toLocaleString('zh-CN')} chars)` : ''}</div>
    <div class="blank" aria-hidden="true"></div>
  </header>
