...
协议。扩展通过本机 Chrome、Edge、Chromium，或 macOS Safari WebDriver 创建隔离的番茄官方登录上下文：

1. 默认在官方页面中切换到“扫码登录”，读取其 data URI 二维码并按原始像素显示到侧边栏；Safari 同时保留官方二维码窗口作为扫描回退。
...
4. 只有官方页面触发滑块时，才恢复 Chromium 验证窗口供用户手动操作；扩展不自动破解验证码。
5. 密码登录保留在官方小窗口内，扩展不接收、读取或保存密码。
6. 检测到登录会话后，先用 `/api/user/info/v2` 验证，等待官方页面补齐会话 Cookie，再用书架接口验证后保存到 VS Code SecretStorage。
7. 同时保留该隔离登录浏览器的 User-Agent；后续用户与书架请求保持相同平台标识，避免 Windows/macOS 之间的会话指纹错配。
8. 无论成功、取消或超时，独立浏览器上下文都会关闭，其临时浏览数据由浏览器自动清理。

Safari 使用 macOS 自带的 `/usr/bin/safaridriver`，需要用户预先启用“允许远程自动化”。Safari
会阻止用户操作 WebDriver 自动化窗口；系统弹窗中的“Continue Session”只会继续自动化，无法
允许手动拖动。短信风控要求滑块时，扩展结束 Safari 会话并提示改用扫码或 Chromium，且不尝试
自动破解或绕过验证。
