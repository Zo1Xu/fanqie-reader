...
4. 只有官方页面触发滑块时，才恢复验证窗口供用户手动操作；Chromium 使用 520×820，Safari 使用 900×800；扩展不自动破解验证码。
...
Safari 使用 macOS 自带的 `/usr/bin/safaridriver`，需要用户预先启用“允许远程自动化”。短信
风控要求滑块时，扩展保留原 Selenium 会话、自动显示较宽的 Safari 验证窗口并持续轮询页面状态；
不同系统或验证码服务可能限制自动化窗口中的鼠标拖动，失败时提示改用扫码或 Chromium，且不
尝试自动破解或绕过验证。
