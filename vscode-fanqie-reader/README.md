...
本扩展通过番茄小说官网读取章节。“番茄小说”等名称、标识和内容归其权利人所有。

## 功能

- 通过书籍 ID、详情页链接或章节链接直接打开公开内容。
- 仅通过官网读取正文；公开章节直接阅读，受限章节使用扩展中保存的账号会话重试。
- 章节显示在 Problems、Output、Terminal 同一区域的 `fanqieReader` Tab，不占用编辑器标签页。
- 阅读区模拟终端输出，顶部提示符和三行 `[INFO]` 状态在滚动时固定。
- 底部上一章、下一章按钮常显，阅读面板标题栏也提供左右箭头；无相邻章节时禁用对应按钮。
- `Alt+PageUp` / `Alt+PageDown` 切换上一章 / 下一章。
- 默认访客阅读；点击“登录并同步书架”才显示登录面板，支持官网扫码、短信验证码和官方小窗口登录。
- 已有账号时，受限章节会自动以该账号重试一次，按服务端返回的阅读权限显示内容。
...
3. 需要个人书架或使用已有账号阅读权限时，点击“登录并同步书架”。可选择扫码、短信验证码或官方小窗口登录，也可点击“暂不登录”关闭面板。
...
- 公开章节优先匿名读取；受限章节才尝试已保存的账号会话。登录成功后自动重试当前可见的受限章节。
- 阅读正文不启动浏览器；浏览器仅用于用户主动发起的登录流程。
...
- 番茄阅读：退出登录
- 番茄阅读：刷新书架
- 番茄阅读：复制阅读诊断（读取失败时用于排查，不包含账号、设备标识或正文）

## 已知限制

- 试读范围外的章节需要账号具有官网网页阅读权限；在浏览器登录后，还需在扩展中登录同一账号。
- 官网仍返回试读或请求失败时会显示原因，不把试读内容标记为全文。
- 网页结构或接口变化可能导致功能失效。
...
扩展源码按 [GNU Affero General Public License v3.0 only](LICENSE) 发布。小说正文、网站内容、商标及第三方资源不属于本许可证授权范围。
